$(function() {
	...
	function onTrendSelectionChange(title) {
						...
						function(result) {
							...
							for ( var title in result) {
								...
								for ( var i in result[title]['pageData']) {
									var rawData = result[title]['pageData'][i]['date'];
									var date = rawData.substring(4, 6) + "-"+ rawData.substring(6, 8) + "-" + rawData.substring(0, 4);
							...
							$('#select-title').val(result[Object.keys(result)[0]].pageTitle);
							onTrendSelectionChange(result[Object.keys(result)[0]].pageTitle);
						});
	}
	...
	(function() {
		...
		titles.push('India');
		titles.push('France');
		titles.push('Singapore');
		reset(titles.join(',,'));
		/*
		 * var title = titles[0]; if (title == '--SELECT--') { return; }
		 * options2.title.text = 'Page Trend for "' + title + '"';
		 * options2.xAxis[0].categories = chartdata[title]['date'];
		 * options2.series = [ { name : 'Page Count', type : 'spline', data :
		 * chartdata[title]['viewCount'],
		 *  }, { name : 'Page Trend', type : 'column', yAxis : 1, data :
		 * chartdata[title]['trend'], } ];
		 * console.log(JSON.stringify(options2)); chart2 = new
		 * Highcharts.Chart(options2);
		 */
	})();
});
